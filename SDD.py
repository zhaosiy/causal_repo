...
originalfolder = '/Users/siyanzhao/Desktop/causalcity/causalcity.github.io/code/SDD/annotations/'
subfolder = []
...
data = scene_data[str(1)]
nbr_car = data.shape[0]
import matplotlib.pyplot as plt

for agent in range(nbr_car):
    this_agent_data = data[agent]
    X_gt = this_agent_data[8:, 0]
    Y_gt = this_agent_data[8:, 1]
    his_X = this_agent_data[:8, 0]
    his_Y = this_agent_data[:8, 1]
    fig = plt.figure()
    ax1 = fig.add_subplot(111)

    path_x = this_agent_data[:, 0]
    path_y = this_agent_data[:, 1]
    print(his_X,'hisx', path_x[:8])
    #assert his_X == path_x[:8]
    x_mean = np.mean(path_x)
    y_mean = np.mean(path_y)
    # plt.xlim(x_mean - 200, x_mean + 200)
    # plt.ylim(y_mean - 200, y_mean + 200)
    ax1.scatter(X_gt, Y_gt, s=10, c='r', marker="s", label='future')
    ax1.scatter(his_X, his_Y, s=10, c='y', marker="s", label='history')

    ax1.scatter(path_x, path_y, s=3, c='b', marker="s", label='whole')
    plt.legend(loc='upper left')
    fig.savefig('sdd_no_egde_predictions/grounttruth/' + 'agent_' + str(agent) + '.png')
    plt.close()
